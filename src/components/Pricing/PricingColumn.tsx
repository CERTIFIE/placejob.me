            ...
            <div className="p-6 border-b border-gray-200 rounded-t-xl">
                ...
                <p className="text-3xl md:text-5xl font-bold mb-6">
                    <span className={clsx({ "text-secondary": highlight })}>
                        {price.month && `$${price.month.price}`}
                        {price.custom && `${price.custom.price}`}
                    </span>
                    {price.month && <span className="text-lg font-normal text-gray-600">/{price.month.period}</span>}
                    {price.custom && <span className="text-lg font-normal text-gray-600">/{price.custom.period}</span>}
                </p>
